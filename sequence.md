...
sequenceDiagram
    User->>Computer: pls geef menu.html
    Computer-->>User: says no
    User->>User: Cry
    User->>AzureFunction: Pls geef mij het menu
    AzureFunction->>BlobContainer: Pls GET 'menu.json'
    BlobContainer-->> AzureFunction: no
    AzureFunction-->>User: Computer says no.
    User->>User: Cry some more
    User->>User: Vraag Yasen?
```